...
async function processExtractionInBackground(contactId: number, buffer: Buffer, mimeType: string) {
  try {
    const apiKey = process.env.GEMINI_API_KEY;
    if (!apiKey) {
      console.error('GEMINI_API_KEY is not configured for background task');
      await db.update(contacts).set({ status: 'error' }).where(eq(contacts.id, contactId));
      return;
    }
    ...
    try {
      ...
      console.error('Failed to parse JSON from Gemini in background:', responseText);
      await db.update(contacts).set({ status: 'error' }).where(eq(contacts.id, contactId));
...
export async function uploadAndExtractAction(formData: FormData) {
  try {
    ...
    processExtractionInBackground(insertedContact.id, buffer, file.type);
